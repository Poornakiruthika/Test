...
module.exports = {
  ...
  'Fullscreen': function ( fullScreen ) {
    ...
    if ( url.length > 0 ) {
      for ( var i = 1, j = 1; i != url.length; i++ ) {
        fullScreen.url ( url[ i ] ).
        playvideo ( ).
        ...
        getAttribute ( ".gadget-artifact-container > div", "fullscreen", function ( getMode ) {
          if ( getMode.value == 'true' ) {
            fullScreen.writeToExcelPass ( 'videoplayer.xlsx', 'Fullscreen', ++j, 2 );
          }
          else {
            ...
            fullScreen.writeToExcelFail ( 'videoplayer.xlsx', 'Fullscreen', ++j, 2, 3, "ActualResult: '" + getMode.value + "'. ExpectedResult: 'true' ( Full screen mode is not working ) " );
          }
        } );
      }
    }
