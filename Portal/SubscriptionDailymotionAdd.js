...
'Deleting the Subscription Title': function( deleteSubscription )  {
    ...
    for ( subIncrementer = 1,temp = 1; subIncrementer <= searchContentDelete.length-1; subIncrementer++ )  {
      deleteSubscription.
      useXpath ().
      ...
      getText ( "//DIV[@class='container']",function( searchResult )
      {
        ...
        if ( searchResult.value != "⬇Last modified\nNo results found\nTry checking your spelling or using more general keywords" )
        {
          ...
          setValue (" ( //INPUT[@autocomplete='off'])[2]",searchContentDelete [ temp ] ).
          pause ( 5000 ).
          ...
          request ( 'https://api.staging.boxxspring.com/properties/81/subscriptions?access_token=c4b4e9a62395f967d5b7738328d12a43&name.like="'+searchContent[j]+'"', function(err, res, body)
          {
